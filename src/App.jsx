      ...
      <div style={docStyle}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24,paddingBottom:16,borderBottom:`2px solid ${gold}`}}>
          <div style={{display:'flex',gap:12,alignItems:'center'}}>
            ...
            <p style={{fontSize:20,fontWeight:700,color:gold,letterSpacing:2}}>KINGDOM PAINTING INC. QUOTE</p>
          </div>
    ...
    <div style={{padding:24,overflow:'auto',maxHeight:'100%',background:'#f5f5f0'}}>
      <div style={docStyle}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24,paddingBottom:16,borderBottom:`2px solid ${gold}`}}>
          <div style={{display:'flex',gap:12,alignItems:'center'}}>
            <img src="/kingdom-logo-dark.svg" alt="Kingdom Painting" style={{height:48}}/>
            <p style={{fontSize:16,fontWeight:700,color:gold,letterSpacing:1}}>KINGDOM PAINTING INC. CONTRACT</p>
          </div>
          <div style={{textAlign:'right'}}>
            <p style={{fontSize:11,color:'#666'}}>{todayStr}</p>
          </div>
...
function MasterEstimate(){
  ...
  const pushToProject=async()=>{
    ...
    try{
      ...
      // Portal contract = the exact Bid Proposal contract (same generator as the Export Bid).
      const chtml=buildBidProposalHtml(client,rooms,settings,totals,ps.paints,ps.ceilPaints,ps.primers,ps.colours,ps.swColours,ps.supplies,__projName,'contract');
...
function buildBidProposalHtml(client,rooms,settings,totals,paints,ceilPaints,primers,colours,swColours,supplies,projectName,mode='full'){
  ...
  if(mode!=='quote'){
  html+='<div class="page">';
  html+=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;padding-bottom:16px" class="border-gold"><div style="display:flex;gap:12px;align-items:center"><img src="${LOGO_PNG}" style="height:48px"><span style="font-size:20px;font-weight:700;color:${gold};letter-spacing:1px">KINGDOM PAINTING INC. CONTRACT</span></div><div style="text-align:right"><p style="font-size:11px;color:#666">${today}</p></div></div>`;
